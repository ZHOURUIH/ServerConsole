...
ShareMemoryServer::ShareMemoryServer()
{
	m_dwLastError = 0;
...
ShareMemoryServer::ShareMemoryServer(char *szFileName, char *szMapName, DWORD dwSize)
{
	_Init();
	Create(szFileName, szMapName, dwSize);
}
// 初始化各個參數
void ShareMemoryServer::_Init()
{
	m_hFile = NULL;
	m_hFileMap = NULL;
...
// 判斷是否NT4.0以上操作系統
bool ShareMemoryServer::_IsWinNTLater()
{
	OSVERSIONINFOA Ver;
...
// 創建共享內存塊
bool ShareMemoryServer::Create(char *szFileName, char *szMapName, DWORD dwSize)
{
	// 釋放已有的共享內存塊
	if (m_bCreateFlag)
	{
...
// 獲取內存文件指針
LPVOID ShareMemoryServer::GetBuffer()
{
	return (m_lpFileMapBuffer) ? (m_lpFileMapBuffer) : (NULL);
}

bool ShareMemoryServer::WriteCmdData(DWORD nCommandCode, DWORD dwDataSize, const LPVOID pBuf)
{
	// 檢驗數據的合理性
	if (NULL == GetBuffer())
	{
